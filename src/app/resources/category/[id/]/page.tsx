...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const categoryName = CATEGORIES[id] || id;
  return {
    title: `${categoryName} Guides & Resources | The Oldverse Productions`,
    description: `Browse all expert guides, filmmaking articles, and career roadmaps related to ${categoryName} on The Oldverse.`,
  };
}
...
export default async function CategoryPage({ params }: Props) {
  const { id } = await params;
  const categoryName = CATEGORIES[id] || id;
  const articles = ARTICLES_REGISTRY.filter((a) => a.category === id);
    ...
    <div className="bg-oldverse-bg min-h-screen pt-28 pb-16 font-sans">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <div className="flex items-center gap-2 text-xs text-white/40 font-grotesk uppercase">
          ...
          <span>/</span>
          <span className="text-white/60">Category: {categoryName}</span>
        </div>
        ...
        <div className="space-y-4 border-b border-white/5 pb-8">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-white/50 font-light">
              Showing {articles.length} guides under the {categoryName} category
            </p>
          </div>
        </div>
        ...
        {articles.length > 0 ? (
              ...
              >
                <div className="space-y-3">
                  <div className="flex items-center gap-4 text-[10px] text-white/40 font-grotesk uppercase">
                    <span className="text-oldverse-accent font-semibold">{categoryName}</span>
          ...
          <div className="text-center py-16 bg-[#0c0c0c]/50 rounded-xl border border-white/5 space-y-4">
            <p className="text-white/50 text-sm font-light">No articles registered under this category yet.</p>
            <Link
              href="/resources"
