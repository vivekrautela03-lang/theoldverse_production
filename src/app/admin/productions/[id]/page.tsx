  ...
  const fetchProduction = async () => {
    try {
      ...
      if (data.success && data.production) {
        ...
        setCrew(p.crew || []);
        setActorsInput((p.actors || []).join(", "));
      } else {
        showToast("Production log not found", "error");
        router.push("/admin/productions");
      }
    } catch {
      showToast("Failed to fetch production details", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        showToast(`Production "${formData.title}" updated!`, "success");
        router.push("/admin/productions");
      } else {
        showToast(data.error || "Failed to update production", "error");
      }
    } catch {
    ...
    return (
      <AdminLayout>
        <div className="p-16 text-center space-y-3 font-grotesk">
          ...
          <p className="text-xs text-[#B8C2CC]">Loading production log...</p>
        </div>
      </AdminLayout>
    );
  }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h2 className="font-bebas text-3xl sm:text-4xl tracking-wider text-white uppercase leading-none">
                Edit Production: {formData.title}
              </h2>
            </div>
          </div>
            ...
            <div className="bg-[#0B0E13] border border-white/5 rounded-2xl p-6 space-y-5 shadow-xl">
              ...
              <div className="space-y-4 text-xs">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    <label className="font-semibold text-white uppercase tracking-wider block">Budget</label>
                    <input
                      type="text"
                ...
                <div className="space-y-1.5">
                  <label className="font-semibold text-white/80 uppercase tracking-wider block">Cast / Actors</label>
                  <input
                    type="text"
                    value={actorsInput}
                ...
                <div className="space-y-1.5">
                  <label className="font-semibold text-white/80 uppercase tracking-wider block">Production Notes</label>
                  <textarea
                    rows={4}
