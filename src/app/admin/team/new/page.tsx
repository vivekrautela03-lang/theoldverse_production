...
export default function NewTeamMemberPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    bio: "",
    avatar_url: "https://images.unsplash.com/photo-1534528741775-53994a69daeb?q=80&w=400&h=400&fit=crop",
    instagram_url: "",
    linkedin_url: "",
    email: "",
    display_order: 0,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name || !formData.position || !formData.bio || !formData.avatar_url) {
      showToast("Name, position, bio, and profile photo are required fields", "warning");
      return;
    }
    ...
    try {
      ...
      if (res.ok && data.success) {
        showToast(`Team member "${formData.name}" added successfully!`, "success");
      ...
      <form onSubmit={handleSubmit} className="space-y-8 font-grotesk max-w-4xl mx-auto">
        ...
        <div className="bg-[#0B0E13] border border-white/5 rounded-2xl p-6 sm:p-8 space-y-6 shadow-xl">
          ...
          <div className="space-y-1.5 text-xs">
            <label className="font-semibold text-white uppercase tracking-wider block">Biography / Short Intro *</label>
            <textarea
              rows={4}
              required
              value={formData.bio}
              onChange={(e) => setFormData({ ...formData, bio: e.target.value })}
              placeholder="Passionate film director and visual storyteller with over 6 years experience..."
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-xs">
            <div className="space-y-2">
              <label className="font-semibold text-white uppercase tracking-wider block">Profile Image URL *</label>
              <input
                type="url"
                required
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  placeholder="member@theoldverse.com"
                  className="w-full px-3.5 py-2 bg-black/40 border border-white/10 rounded-xl text-white focus:outline-none focus:border-[#8DBEFF]"
                />
              </div>
            </div>
