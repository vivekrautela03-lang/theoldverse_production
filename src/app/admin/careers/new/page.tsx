...
export default function NewJobPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    description: "",
    requirements: "3+ years editing experience, proficiency in Premiere/DaVinci, portfolio required",
    type: "Full-Time",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.title || !formData.department || !formData.description) {
      showToast("Title, department, and description are required fields", "warning");
      return;
    }
    ...
    try {
      ...
      if (res.ok && data.success) {
        showToast(`Job opening "${formData.title}" published!`, "success");
        router.push("/admin/careers");
      } else {
        showToast(data.error || "Failed to post job opening", "error");
      }
    } catch {
    ...
    <AdminLayout>
      <form onSubmit={handleSubmit} className="space-y-8 font-grotesk max-w-4xl mx-auto">
        ...
        <div className="bg-[#0B0E13] border border-white/5 rounded-2xl p-6 sm:p-8 space-y-6 shadow-xl text-xs">
          ...
          <div className="space-y-1.5">
            <label className="font-semibold text-white uppercase tracking-wider block">Job Location</label>
            <input
              type="text"
          ...
          <div className="space-y-1.5">
            <label className="font-semibold text-white uppercase tracking-wider block">Role Description *</label>
            <textarea
              rows={4}
              required
              value={formData.description}
              onChange={(e) => setFormData({ ...formData, description: e.target.value })}
              placeholder="Detailed description of responsibilities, creative expectations..."
          ...
          <div className="space-y-1.5">
            <label className="font-semibold text-white/80 uppercase tracking-wider block">Key Requirements (Comma Separated)</label>
            <textarea
              rows={3}
